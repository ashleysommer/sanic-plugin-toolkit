...
class SanicPluginRealm(object):
    ...
    def _register_helper(plugin, context, *args, _realm=None, _plugin_name=None, _url_prefix=None, **kwargs):
        error_str = "Plugin must be initialised using the " "Sanic Plugin Toolkit PluginRealm."
        assert _realm is not None, error_str
        assert _plugin_name is not None, error_str
        _app = _realm._app
        assert _app is not None, error_str

        reg = PluginRegistration(realm=_realm, plugin_name=_plugin_name, url_prefix=_url_prefix)
    ...
    def _plugin_register_middleware(
        ...
        **kwargs,
        ...
        assert 0 <= priority <= 9, (
            "Priority must be between 0 and 9 (inclusive), " "0 is highest priority, 9 is lowest."
        )
        assert isinstance(plugin, SanicPlugin), "Plugin middleware only works with a plugin from SPTK."
        if len(args) > 0 and isinstance(args[0], str) and attach_to is None:
            ...
            attach_to = args[0]
        if with_context:
            middleware = update_wrapper(partial(middleware, context=context), middleware)
        if attach_to is None or attach_to == "request":
            insert_order = len(self._pre_request_middleware) + len(self._post_request_middleware)
            ...
            priority_middleware = (priority, insert_order, middleware)
            assert relative is None, "A cleanup middleware cannot have relative pre or post"
            ...
            insert_order = len(self._post_response_middleware) + len(self._pre_response_middleware)
    ...
    def get_from_context(self, item, context=None):
        ...
        try:
            ...
            logger.warning("Context {:s} does not exist! Falling back to shared context".format(context))
    ...
    def create_temporary_request_context(self, request):
        ...
        shared_requests_dict[request_hash] = shared_request_ctx = SanicContext(
            self, None, {'request': request, 'id': "shared request context for request {}".format(id(request))}
        )
        for name, _p in self._plugins_context.items():
            if not (isinstance(_p, SanicContext) and 'instance' in _p and isinstance(_p['instance'], SanicPlugin)):
                continue
    ...
    async def _handle_request(self, real_handle, request, write_callback, stream_callback):
        cancelled = False
        try:
            ...
            logger.error("SPTK caught an error that should have been caught by Sanic response handler.")
            logger.error(str(be))
            raise
        finally:
    ...
    async def _run_request_middleware_18_12(self, request):
        if not self._running:
            raise ServerError("Toolkit processing a request before App server is started.")
        ...
        if not self._running:
            ...
            if self._app.asgi:
                if test_mode:
                    ...
                    self.warning("Unexpected ASGI request. Forcing Toolkit " "into running mode without a server.")
                self._on_server_start(request.app, request.transport.loop)
            elif test_mode:
                self.warning("Unexpected test-mode request. Forcing Toolkit " "into running mode without a server.")
                self._on_server_start(request.app, request.transport.loop)
            else:
                raise RuntimeError("Sanic Plugin Toolkit received a request before Sanic server is started.")
        self.create_temporary_request_context(request)
        if self._pre_request_middleware:
            for (_pri, _ins, middleware) in self._pre_request_middleware:
                response = middleware(request)
    ...
    async def _run_request_middleware_21_03(self, request, request_name=None):
        ...
        realm_request_middleware_started = shared_req_context.get('realm_request_middleware_started', False)
    ...
    async def _run_response_middleware_19_12(self, request, response, request_name=None):
        if self._pre_response_middleware:
            for (_pri, _ins, middleware) in self._pre_response_middleware:
                _response = middleware(request, response)
                if isawaitable(_response):
    ...
    async def _run_response_middleware_21_03(self, request, response, request_name=None):
        if self._pre_response_middleware:
            for (_pri, _ins, middleware) in self._pre_response_middleware:
                _response = middleware(request, response)
                if isawaitable(_response):
    ...
    def _recreate(cls, app):
        ...
        self._contexts['_plugins'] = SanicContext(self, None, {'sanic_plugin_toolkit': self})
        return self
    ...
    def __init__(self, *args, **kwargs):
        ...
        assert self._app and self._contexts, "Sanic Plugin Realm was not initialized correctly."
        assert len(args) < 1, "Unexpected arguments passed to the Sanic Plugin Realm."
        assert len(kwargs) < 1, "Unexpected keyword arguments passed to the SanicPluginRealm."
        super(SanicPluginRealm, self).__init__(*args, **kwargs)

    def __getstate__(self):
        if self._running:
            raise RuntimeError("Cannot call __getstate__ on an SPTK app that is already running.")
        state_dict = {}
    ...
    def __setstate__(self, state):
        ...
        if running:
            raise RuntimeError("Cannot call __setstate__ on an SPTK app that is already running.")
    ...
    def __reduce__(self):
        if self._running:
            raise RuntimeError("Cannot pickle a SPTK PluginRealm App after it has started running!")
